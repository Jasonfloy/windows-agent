...
def getRelease():
	...
	release = re.split('\n')[1] 
        ...
        re = osAction('systeminfo | findstr "os"').decode('GBK') #如果主机不能使用wmic命令，就用systeminfo取
        release = (((re.split('\n')[0]).split(':'))[1]).lstrip()   
    return release
...
def memMon():
    '''内存监控，取出的数据大小单位为字节'''
    mon_data = {}
    rt = psutil.phymem_usage() #取物理内存使用率
...
def netMon():
    '''网络监控，取出的数据大小单位为字节'''
    mon_data={}
    data = psutil.network_io_counters(pernic=False) #取网卡流量信息
